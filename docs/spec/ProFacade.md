...
| ------------------------------ | ------------------------------------------------------------ |
| `typename F::super_types`      | A [tuple-like](https://en.cppreference.com/w/cpp/utility/tuple/tuple-like) type that contains any number of types `Ss`. Each type `S` in `Ss` shall meet the *ProFacade* requirements of `P`. |
...
Additionally, for each convention `C` of every super of `F`, reachable via `typename F::super_types` transitively, where `typename C::overload_type` is a specialization of [`facade_aware_overload_t`](facade_aware_overload_t.md), `C` shall meet the [*ProConvention* requirements](ProConvention.md) of `P` with [`substituted-overload`](ProOverload.md)`<typename C::overload_type, F>` in place of `typename C::overload_type`. That is, a facade-aware convention inherited from a super is checked against `F` as well as against the super that declares it, because the two substitute to different overloads.
