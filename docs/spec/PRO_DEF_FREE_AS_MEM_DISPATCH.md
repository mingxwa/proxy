...
`(2)` Defines a class named `dispatch_name` of free function call expressions of `func_name` with accessibility via member function overloads named `accessibility_func_name`. The accessibility is provided by the nested class `dispatch_name::access_type` (see [*ProAccessible* requirements](ProAccessible.md)). Effectively equivalent to:
