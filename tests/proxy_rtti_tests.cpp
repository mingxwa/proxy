...
namespace proxy_rtti_tests_detail {
...
namespace detail = proxy_rtti_tests_detail;

TEST(ProxyRttiTests, TestIndirectCast_Ref_Succeed) {
  int v = 123;
  pro::proxy<detail::TestFacade> p = &v;
...
TEST(ProxyRttiTests, TestIndirectCast_Copy_Succeed) {
  int v1 = 123;
  pro::proxy<detail::TestFacade> p = &v1;
...
TEST(ProxyRttiTests, TestIndirectCast_Move_Succeed) {
  ...
  auto p = pro::make_proxy<detail::TestFacade>(v1);
...
TEST(ProxyRttiTests, TestIndirectCast_Move_Fail) {
  auto p = pro::make_proxy<detail::TestFacade>(std::vector<int>{1, 2, 3});
  bool exception_thrown = false;
  try {
