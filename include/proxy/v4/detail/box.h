...
template <class F>
using box_accessor_t = composite_t<
    specialization_t<conv_accessors_t,
                     typename facade_traits<F>::indirect_conv_groups, box<F>,
                     F>,
    specialization_t<refl_accessors_t,
                     typename facade_traits<F>::indirect_refls, box<F>>>;

} // namespace detail

template <facade F>
class box : public detail::box_accessor_t<F> {
  template <facade F2>
  friend class box;
