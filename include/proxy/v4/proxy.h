...
public:
  ...
  // Memberwise like the copy constructor; the signed metadata's total copy keeps
  // it correct under PAC, including for an empty proxy.
...
private:
  ...
  constexpr P& initialize(Args&&... args) {
    ...
    if constexpr (proxiable<P, F>) {
      meta_ = typename details::facade_traits<F>::meta{std::in_place_type<P>};
    } else {
  ...
  typename details::facade_traits<F>::meta meta_;
