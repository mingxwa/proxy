...
template <class T>
  requires((std::is_move_constructible_v<T> && std::is_destructible_v<T>) ||
           relocatability_traits<T, constraint_level::trivial>::value)
...
template <class T>
  requires((std::is_trivially_move_constructible_v<T> &&
           ...
           is_bitwise_trivially_relocatable_override<T>::value)
...
concept invocable_dispatch =
    ...
    (NE && std::is_nothrow_invocable_r_v<
               R, D, proxy_arg_t, add_qualifier_t<std::conditional_t<IsDirect, proxy<F>, proxy_indirect_accessor<F>>, Q>, Args...>) ||
    (!NE && std::is_invocable_r_v<R, D, proxy_arg_t,
                                  add_qualifier_t<std::conditional_t<IsDirect, proxy<F>, proxy_indirect_accessor<F>>, Q>, Args...>);
...
public:
  ...
  ~weak_compact_ptr() noexcept {
    if (ptr_ != nullptr &&
        ptr_->weak_count.fetch_sub(1u, std::memory_order::acq_rel) == 1) {
  ...
  strong_compact_ptr<T, Alloc> lock() const noexcept {
    ...
    do {
      if (ref_count == 0) {
        return strong_compact_ptr<T, Alloc>{nullptr};
      }
...
template <class... Cs>
  requires(
      std::is_same_v<typename Cs::dispatch_type, weak_conversion_dispatch> ||
      ...)
struct weak_ownership_support_traits_impl<Cs...> : applicable_traits {};
template <class F>
struct weak_ownership_support_traits
    : instantiated_t<weak_ownership_support_traits_impl,
                     typename F::convention_types> {};
...
struct add_upward_conversion_conv
    : std::type_identity<
          add_conv_t<Cs, conv_impl<true, upward_conversion_dispatch,
...
struct add_upward_conversion_conv<Cs, F, constraint_level::none,
...
template <class O>
struct weak_upward_conversion_overload_traits;
template <class F>
...
template <class... Os>
using weak_upward_conversion_conv =
