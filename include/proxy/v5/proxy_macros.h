...
#define PRO5D_THROW(...) throw __VA_ARGS__
#else
#define PRO5D_THROW(...) std::abort()
#endif // __cpp_exceptions >= 199711L

#ifdef _MSC_VER
#define PRO5D_ENFORCE_EBO __declspec(empty_bases)
#else
#define PRO5D_ENFORCE_EBO
#endif // _MSC_VER
...
#define PRO5D_DEF_OVERLOAD_SPECIALIZATIONS(macro, ...)                         \
  macro(, &, , __VA_ARGS__);                                                   \
...
#define PRO5D_EXPAND_MACRO_IMPL(macro, _1, _2, _3, name, ...) macro##_##name
#define PRO5D_EXPAND_MACRO(macro, ...)                                         \
  PRO5D_EXPAND_IMPL(                                                           \
...
#define PRO5D_DEF_FREE_DISPATCH_3(name, impl, func)                            \
  PRO5D_DEF_FREE_DISPATCH_IMPL(name, impl, func)
#define PRO5_DEF_FREE_DISPATCH(name, ...)                                      \
  PRO5D_EXPAND_MACRO(PRO5D_DEF_FREE_DISPATCH, name, __VA_ARGS__)
...
#define PRO5D_DEF_FREE_AS_MEM_DISPATCH_3(name, impl, func)                     \
  PRO5D_DEF_FREE_AS_MEM_DISPATCH_IMPL(name, impl, func)
#define PRO5_DEF_FREE_AS_MEM_DISPATCH(name, ...)                               \
  PRO5D_EXPAND_MACRO(PRO5D_DEF_FREE_AS_MEM_DISPATCH, name, __VA_ARGS__)

// Version-less macro aliases

#define PRO5D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(name, qualified_name)          \
...
#define PRO_DEF_MEM_DISPATCH(...)                                              \
  PRO5D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(PRO_DEF_MEM_DISPATCH,                \
                                          PRO5_DEF_MEM_DISPATCH)
#else
#define PRO_DEF_MEM_DISPATCH(name, ...) PRO5_DEF_MEM_DISPATCH(name, __VA_ARGS__)
#endif // PRO_DEF_MEM_DISPATCH

#ifdef PRO_DEF_FREE_DISPATCH
#undef PRO_DEF_FREE_DISPATCH
#define PRO_DEF_FREE_DISPATCH(...)                                             \
  PRO5D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(PRO_DEF_FREE_DISPATCH,               \
                                          PRO5_DEF_FREE_DISPATCH)
#else
#define PRO_DEF_FREE_DISPATCH(name, ...)                                       \
  PRO5_DEF_FREE_DISPATCH(name, __VA_ARGS__)
...
#define PRO_DEF_FREE_AS_MEM_DISPATCH(...)                                      \
  PRO5D_AMBIGUOUS_MACRO_DIAGNOSTIC_ASSERT(PRO_DEF_FREE_AS_MEM_DISPATCH,        \
                                          PRO5_DEF_FREE_AS_MEM_DISPATCH)
#else
