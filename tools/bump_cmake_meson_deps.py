...
"""Bump the C++ dependencies that no Renovate manager understands.
...
The script is self-contained: it discovers versions and computes hashes itself. Any problem --
a lookup or download that fails, a missing tool, a failed subprocess -- is a hard error: the
script exits non-zero and the workflow fails.
